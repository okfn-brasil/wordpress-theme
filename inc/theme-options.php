...
//        );
    ...
    add_settings_field(
            "okfnwp_channel_id", // id
            __('Channel Page', 'okfnwp'), // title
            array($this, 'okfnwp_channel_id_callback'), // callback
            'theme-options-admin', // page
            'theme_options_setting_section_social' // section
    );
...
//        }
    ...
    if (isset($input['okfnwp_channel_id'])) {
      $sanitary_values['okfnwp_channel_id'] = sanitize_text_field($input['okfnwp_channel_id']);
    }
...
//    }
  ...
  public function okfnwp_channel_id_callback() {
    $current_val = $this->theme_options['okfnwp_channel_id'];
    $old_val = get_option('okfnwp_channel_id');

    if (!isset($current_val) && isset($old_val)):
      $current_val = $old_val;
    endif;

    printf('<input class="regular-text" type="text" name="theme_options_option_name[okfnwp_channel_id]" id="okfnwp_channel_id" value="%s">', isset($current_val) ? esc_attr($current_val) : '');
    ?>
    <p><?php _e('Channel page. Full url.'); ?></p>
    <?php
  }
